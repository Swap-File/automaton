...
void update_led(void) {
  const int led_blink = 100;
  static uint32_t ledr_time = 0;
  static uint32_t ledg_time = 0;
  static uint32_t ledb_time = 0;
  ...
  if (get_battery() < 3.6) {
    ...
    if (peripheral.connected()) {
      if (hand) {
        digitalWrite(LEDB, payload[0] & 0x01);
      } else {
        digitalWrite(LEDB, payload[0] & 0x01);
      }
    } else {
      if (hand) {
        digitalWrite(LEDB, millis() >> 8 & 0x01);
      } else {
        digitalWrite(LEDB, millis() >> 8 & 0x01);
      }
    }
    ...
    if (peripheral.connected()) {
      ...
      if (hand) {
        digitalWrite(LEDR, millis() >> 8 & 0x01);
      } else {
        digitalWrite(LEDG, millis() >> 8 & 0x01);
      }
    }
  }
}
